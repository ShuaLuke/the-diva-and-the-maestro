...
Thirteen directions for the home page, side by side. Designs 1 to 6 explore period and mood; 7 to 10 start from the audience and from what the best book, editorial, museum and documentary sites already do; 11 to 13 are landing pages for the channels that send readers: BookTok, newsletters, and music. The switcher's "Audience & gold standard" button holds that thinking and a recommendation. The chooser is published with the site at `/design-choices/` (linked from the Plan page) and is behind the same password. The HTML files are hand-written and use web-sized copies of the archive photographs in `img/`.

Open `index.html` in a browser (double-click it in Finder, or run the dev server and visit `/the-diva-and-the-maestro/design-choices/`) and click between the designs, or use the arrow keys or the number keys. The notes bar under the tabs gives each design's vibe, rationale, audience and trade-offs; "Hide notes" collapses it and the N key toggles it. Each design can also be opened on its own.
...
| 11 | BookTok | Dark academia, phone-first: a 9:16 trailer frame, swipeable story cards, trope tags, a creator kit. |
| 12 | Dispatch | A Substack-style newsletter from the authors: a feed of finds from the box, subscribe box on top. |
| 13 | Playlist | A record with liner notes: all twelve recordings as a working playlist with a player bar. |

Fonts load from Google Fonts, so the pages need an internet connection to look right. Designs 7, 8, 10, 12 and 13 play recordings from `public/audio/`, which works on the published site and the dev server and may be blocked when opened straight from Finder.
